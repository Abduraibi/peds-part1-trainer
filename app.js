...
/* Open ANY calendar day (past, today, or future). Builds its plan on demand,
   lets the user study it, and saves completion to that exact date. */
function openDay(d){
  const today = todayStr();
  if(d === today){ switchView("plan"); return; }   // today → normal plan view

  // Build (or reuse) the frozen plan for this date so future days get a real plan
...
function renderRunner(){
  ...
  if(prev){
    ...
    if(log&&log.done&&log.done[q.id]){
      // already completed on this day in an earlier session — show answer + explanation
      const correctLab=(q.answer.match(/^([A-G])/)||[])[1];
      revealAnswer(q, correctLab||"", true);
      const note=$("#ansLine");
      if(note&&correctLab) note.innerHTML=`✓ Already completed · Correct answer: <b>${esc(q.answer)}</b>`;
    }
  }
}
