...
/* Get today's date (YYYY-MM-DD) in Riyadh timezone (UTC+3).
   IMPORTANT: do NOT build a Date from a locale string and call toISOString() —
   that converts back to UTC and can land on the wrong calendar day depending on
   the browser's own timezone. Instead read the Riyadh Y/M/D parts directly. */
const todayStr = ()=> {
  ...
  return `${get('year')}-${get('month')}-${get('day')}`;  // en-CA → YYYY-MM-DD
};
...
async function onLogin(user){
  ...
  await loadState();
  await ensureTodayPlan(); // archive old days + build today's fresh plan, persisted now
  rolloverDay();
  switchView("dashboard");
}

/* Save state synchronously, bypassing the debounce. Use whenever a change
   MUST survive an immediate page reload (e.g. day rollover). */
async function saveStateNow(){
  ...
  if(sb && USER){
    ...
    }catch(e){ console.warn("saveStateNow error", e); }
  }
}

/* SINGLE authoritative day-rollover function.
   Call this anywhere we might be on a new day. It:
     1. archives every plan whose own .day != today (history is preserved),
     2. removes any leftover non-today slot keyed under today,
     3. builds today's fresh plan if one doesn't already exist,
     4. persists synchronously so a reload can't resurrect stale state.
...
async function ensureTodayPlan(){
  ...
  // 1+2. Archive anything that isn't a real plan for today.
  Object.keys(STATE.dayLog).forEach(d=>{
    const log = STATE.dayLog[d];
    if(!log) return;
    const isTodayPlan = (log.day === today);
    if(!isTodayPlan && log.groups && !log.archived){
      log.archived = true; changed = true;          // keep for history slider
    }
    // A slot keyed under today's date but whose .day is wrong/stale: drop it
    if(d === today && log.day !== today){
      delete STATE.dayLog[today]; changed = true;
    }
  });

  // 3. Build today's plan if missing or only an archived shell exists.
  const t = STATE.dayLog[today];
  const haveLivePlan = t && t.groups && !t.archived && t.day === today;
  if(!haveLivePlan){
    buildDayPlan(currentPlanMode||"full"); // writes STATE.dayLog[today] + saveState()
    changed = true;
  }

  // 4. Persist synchronously so reload sees the clean state.
  if(changed) await saveStateNow();
  return changed;
}

/* Back-compat shim: old callers still reference purgeStaleTodayPlan. */
async function purgeStaleTodayPlan(){ return ensureTodayPlan(); }
...
async function forceNewDay(){
  const today=todayStr();
  // Force a clean rebuild: archive every existing plan, drop today's slot,
  // then let ensureTodayPlan build + persist a fresh one synchronously.
  Object.keys(STATE.dayLog||{}).forEach(d=>{
...
setInterval(async()=>{
  ...
  if(now!==_lastKnownDate){
    ...
    if(STATE){
      await ensureTodayPlan();
      // Re-render whichever view is currently visible so the user sees the new day
...
function recordResult(id,correct){
  ...
  // Anchor on the Riyadh "today" string, add interval days in UTC-noon space
  // to avoid any timezone drift, then read back the date parts.
  const base=new Date(todayStr()+"T12:00:00Z");
  base.setUTCDate(base.getUTCDate()+iv);
  s.nextDue=base.toISOString().slice(0,10);
  STATE.seen[id]=s;
...
function renderDashboard(){
  ...
  // Banner shows ONLY if there is no live plan whose own .day === today.
  // (Archived history entries are ignored — they never trigger it.)
  const today=todayStr();
  const t=STATE.dayLog&&STATE.dayLog[today];
  const hasTodayPlan = !!(t && t.groups && !t.archived && t.day===today);
