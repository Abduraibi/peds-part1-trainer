...
/* RECOVERY: rebuild each day's completion from intact global answer history.
   A previous migration wiped per-day `done` maps. STATE.seen still holds every
   answered question. For each saved day, any question in that day's plan that
   you've actually answered (exists in STATE.seen) is restored as done.
   Non-destructive: only ADDS back done markers, never clears them. */
function repairDayProgress(){
  if(!STATE || !STATE.dayLog) return;
  let changed=false;
  Object.values(STATE.dayLog).forEach(log=>{
    ...
    log.groups.forEach(g=>(g.itemIds||[]).forEach(id=>{
      if(STATE.seen && STATE.seen[id] && !log.done[id]){
        log.done[id]=true; changed=true;
      }
    }));
  });
  if(changed) saveState();
...
function buildDayPlan(mode, targetDate){
  ...
  // Reuse frozen plan if already built for this day (lists stay stable)
  ...
  const specReview = pickSpecialtyAny(specialty, 8, used)
    .filter(q => STATE.seen[q.id] && !isMastered(q.id));
  ...
  // ── FREEZE (done map carried over; NOT auto-filled from global seen) ──
  STATE.dayLog[day] = {
    day,
    specialty,
...
/* Open ANY calendar day (past, today, or future). Builds its plan once,
   lets the user study it, and saves completion to that exact date. */
function openDay(d){
  const today = todayStr();
  if(d === today){ switchView("plan"); return; }   // today → normal plan view
  // Build the plan once if it doesn't exist yet; otherwise reuse frozen list
  if(!STATE.dayLog[d] || !STATE.dayLog[d].groups){
    buildDayPlan(currentPlanMode||"full", d);
  }
  renderDayView(d);
}
/* Backward-compatible alias */
function openPastDay(d){ renderDayView(d); }
...
function renderRunner(){
  ...
  // restore answered state: this session's pick, or a completion logged on this day
  const prev=runnerCtx.answered[q.id];
  if(prev){
    revealAnswer(q,prev,true);
  }else if(runnerCtx.origin==="plan"||runnerCtx.origin==="pastday"){
    ...
    if(log&&log.done&&log.done[q.id]){
      ...
      const note=$("#ansLine");
      if(note&&correctLab) note.innerHTML=`✓ Already completed · Correct answer: <b>${esc(effectiveAnswer(q))}</b>`;
    }
  }
}
