...
/* One-time cleanup of day-completion markers corrupted by an earlier auto-sync.
   Clears only the per-day `done` maps so day progress recomputes honestly.
   Global progress (seen, mastery, accuracy, review, wrong loop) is untouched. */
function migrateState(){
  if(!STATE) return;
  if((STATE.schema||0) < 4){
    Object.values(STATE.dayLog||{}).forEach(log=>{ if(log) log.done = {}; });
    STATE.schema = 4;
    saveState();
  }
}
