...
async function onLogin(user){
  ...
  // Inject settings gear next to logout
...
function openSettings(){
  ...
  bd.innerHTML=`
    <div class="modal" style="max-width:460px">
      ...
      <label style="display:block;font-size:13px;color:var(--ink-soft);margin-bottom:6px">NEW EXAM DATE</label>
      <input type="date" id="settingsExamDate" value="${curExam}" min="${today}"
        style="width:100%;padding:10px 14px;border:2px solid var(--border);border-radius:10px;font-size:15px;background:var(--card);color:var(--ink);box-sizing:border-box;margin-bottom:10px">
...
async function saveSettings(){
  ...
  STATE.examDate=val;
  STATE.useFixedSchedule=false;
  // Reset planStart to today so day count starts from 1
  STATE.planStart=todayStr();
  const today=todayStr();
  const preservedDone=(STATE.dayLog[today]||{}).done||{};
  // Wipe ALL frozen plans so everything rebuilds fresh from new dates
