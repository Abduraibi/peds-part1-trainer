...
function hasEdits(id){
  const e=getEdit(id);
  return !!(e.answer||e.stem!==undefined||e.notes||e.explanation!==undefined);
}
...
function saveEdit(id){
  ...
  if(!e.answer && e.stem===undefined && !e.notes && e.explanation===undefined) delete STATE.edits[id];
  saveState();
  closeEditModal();
...
function coachAdvice(s){
  ...
  const pace=Math.ceil((s.total-s.seen)/Math.max(1,daysBetween(todayStr(),FINISH_BY)+1));
  let message="";
...
function revealAnswer(q,lab,restore){
  ...
  if(correctLab){
    ...
    $("#ansLine").innerHTML=`✓ Correct answer: <b>${esc(effAns)}</b>${editNote}
      ${!restore?`<button class="btn ghost sm" style="margin-left:12px;font-size:12px" onclick="changeAnswer('${q.id}')">↩ Change answer</button>`:''}`;
  }else{
