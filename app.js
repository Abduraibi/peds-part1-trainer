...
function getTodayScheduleEntry(){
  return dateToScheduleEntry(todayStr());
}

/* Get the schedule entry for any calendar date (past, today, or future) */
const PLAN_START_DATE = "2026-06-02";
function dateToScheduleEntry(dateStr){
  const dayNum = daysBetween(PLAN_START_DATE, dateStr) + 1; // 1-indexed
...
function buildDayPlan(mode, targetDate){
  ...
  // Answered questions on THIS day stay anchored (never dropped, never lost).
  const doneMap = (saved && saved.done) || {};
  const anchored = new Set(Object.keys(doneMap).filter(id => QBY[id]));

  // Determine this day's specialty from the 35-day schedule
  const schedEntry  = dateToScheduleEntry(day);
  const specialty   = schedEntry.spec;
  const specMeta    = SPECIALTY_META[specialty] || { icon:"📋", color:"spec-gen" };

  // map every anchored id to the group key it belongs in, so we can re-insert it
  const prevGroupOf = {};
  if(saved && saved.groups){
    saved.groups.forEach(g => (g.itemIds||[]).forEach(id => { if(anchored.has(id)) prevGroupOf[id] = g.key; }));
  }
  // anchored ids whose original group we know
  const anchoredByKey = key => [...anchored].filter(id => prevGroupOf[id] === key);
  // anchored ids with no known group (fallback → goes to "new")
  const anchoredOrphans = () => [...anchored].filter(id => !prevGroupOf[id]);

  const used   = [];
  const groups = [];
  // helper: build a group's final item list = anchored-for-this-group (kept) + fresh picks
  const compose = (key, freshItems) => {
    const keep = anchoredByKey(key).map(id => QBY[id]).filter(Boolean);
    const keepIds = new Set(keep.map(q => q.id));
    const fresh = freshItems.filter(q => !keepIds.has(q.id) && !anchored.has(q.id));
    return [...keep, ...fresh];
  };

  // ── 1. Memory Review (spaced repetition due) ─────────────────────────
  const sr = compose("memory", dueForReview().slice(0, 25));
  ...
  const wl = compose("wrongloop", pendingWrongLoop().filter(q => !used.includes(q.id)).slice(0, 30));
  ...
  // ── 3. Specialty Focus P1 — must-master for this day's domain ────────
  const specP1 = compose("spec_p1", pickSpecialtyPrio(specialty, "P1", 20, used));
  ...
  const specP2 = compose("spec_p2", pickSpecialtyPrio(specialty, "P2", 12, used));
  ...
  const specReview = compose("spec_review",
    pickSpecialtyAny(specialty, 8, used).filter(q => STATE.seen[q.id]));
  ...
  // include any anchored orphans here so completed-but-unclassified items survive
  const orphanItems = anchoredOrphans().map(id => QBY[id]).filter(Boolean);
  const freshFill   = pickNew(fillCount, used);
  const fill        = [...orphanItems, ...freshFill.filter(q => !anchored.has(q.id))];
  fill.forEach(q => used.push(q.id));
  if(fill.length) groups.push({
    key:  "new",
    title: "✨ New Questions",
    sub:  `${freshFill.length} new questions across all specialties to hit today's target of ${quota}`,
    icon: "✨",
    items: fill
  });
  ...
  // Re-sync day progress: any question in this day's list that's been answered
  // on ANY day (global STATE.seen) counts as done for this day too.
  const syncedDone = {...doneMap};
  const allListed = uniq(groups.flatMap(g => g.items.map(q => q.id)));
  allListed.forEach(id => { if(STATE.seen[id]) syncedDone[id] = true; });

  // ── SAVE (re-synced; answered items preserved via `done`) ────────────
  STATE.dayLog[day] = {
    day,
    specialty,
    schedDay: schedEntry.day,
    landmark: schedEntry.landmark || null,
    archived: (saved && saved.archived) || false,
    fudul,
    used,
    busyP1Ids,
    done: syncedDone,
...
function toggleFudulDone(){
  ...
  if(alreadyDoneToday){
    // unmark — subtract exactly the batch we added (fall back to today's target)
    const batch = STATE.fudulLastBatch || f.dailyTarget;
    STATE.fudulDone=Math.max(0,STATE.fudulDone-batch);
    STATE.fudulDoneDate=null;
    STATE.fudulLastBatch=0;
    saveState(); renderPlan();
    toast(`Fudul marked not done — back to ${STATE.fudulDone}/${f.total}`);
  }else{
    // mark done — remember how many we added so unmark is exact
    const batch=Math.min(f.dailyTarget, f.total-STATE.fudulDone);
    STATE.fudulDone=Math.min(f.total,STATE.fudulDone+batch);
    STATE.fudulDoneDate=todayStr();
    STATE.fudulLastBatch=batch;
    saveState(); renderPlan();
  ...
  <div class="card" style="margin-bottom:18px;padding:18px 20px;border-left:4px solid var(--gold)">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:14px;flex-wrap:wrap">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;flex-shrink:0;align-items:flex-end">
        <span class="pill" style="background:${plan.fudul.doneToday?'var(--green)':'var(--line)'};color:${plan.fudul.doneToday?'#fff':'var(--ink-soft)'};font-weight:700">${plan.fudul.doneToday?'\u2713 Done':'\u25cb Not done'}</span>
        ...
        ${plan.fudul.doneSoFar<plan.fudul.total?
          `<button class="btn sm" onclick="toggleFudulDone()" style="background:${plan.fudul.doneToday?'var(--green)':'var(--gold)'};border-color:${plan.fudul.doneToday?'var(--green)':'var(--gold)'}">
            ${plan.fudul.doneToday?'\u2713 Done (tap to mark not done)':'Mark today done ('+plan.fudul.from+'\u2013'+plan.fudul.to+')'}
          </button>`
...
/* Open ANY calendar day (past, today, or future). Re-syncs its plan against
   your live global progress each time, keeps answered questions anchored,
   and saves completion to that exact date. */
function openDay(d){
  const today = todayStr();
  if(d === today){ switchView("plan"); return; }   // today → normal plan view

  // Always rebuild/re-sync so the day reflects progress from every other day.
  // Answered questions are anchored inside buildDayPlan and never lost.
  buildDayPlan(currentPlanMode||"full", d);
  renderDayView(d);
}
/* Backward-compatible alias — re-sync then render */
function openPastDay(d){
  if(STATE.dayLog[d] && STATE.dayLog[d].groups) buildDayPlan(currentPlanMode||"full", d);
  renderDayView(d);
}
...
function renderDayView(d){
  ...
  const today=todayStr();
  const isFuture = d>today, isPast = d<today, isToday = d===today;
  const tag = isFuture?"Future Day — study early":isPast?"Past Day Review":"Today";
  const done=log.done||{};
  const allIds=uniq(log.groups.flatMap(g=>g.itemIds||[]));
  const doneCount=allIds.filter(id=>done[id]).length;
  const total=allIds.length;
  ...
  const sched=dateToScheduleEntry(d);
  ...
  <div class="page-head">
    ...
    <div class="eyebrow">${tag} · Day ${sched.day} · ${esc(sched.spec)}${sched.landmark?' · '+esc(sched.landmark):''}</div>
    <h1>${label} ${complete?'<span class="pill common">✓ Finished</span>':''}</h1>
    <p>${doneCount} of ${total} questions completed (${pct}%)${isFuture?' · You can answer these ahead of time — progress is saved to this day.':''}</p>
  </div>
  <div class="card stat" style="margin-bottom:20px">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
      ...
      <div style="display:flex;gap:9px">
        <button class="btn" onclick='runDayAll("${d}")'>${complete?'Review all →':'Study all questions →'}</button>
        ${!complete?`<button class="btn ghost" onclick='runDayRemaining("${d}")'>Study incomplete →</button>`:''}
      </div>
    </div>
    <div class="progress-bar" style="margin-top:14px"><span style="width:${pct}%"></span></div>
  </div>
  ${log.groups.map(g=>{
    ...
    const gd=ids.filter(id=>done[id]).length;
    const gComplete=gd===ids.length&&ids.length>0;
    return `<div class="task-group">
      <div class="tg-head"><h3>${g.icon||'📋'} ${g.title}</h3><span class="count">· ${gd} of ${ids.length} done</span></div>
      <div class="tasklist" style="margin-top:10px">
        <div class="task ${gComplete?'done':''}">
          <div class="check ${gComplete?'on':''}">${gComplete?'✓':''}</div>
          <div class="t-body">
            <div class="t-title">${g.title}</div>
            <div class="t-sub">${gd} completed · ${ids.length-gd} remaining</div>
          </div>
          <button class="btn sm t-go" onclick='runDayGroup("${d}","${g.key}")'>
            ${gComplete?'Review again':'Study →'}
          </button>
        </div>
      </div>
    </div>`;
...
function runDayGroup(d,key){
  const log=STATE.dayLog[d]; if(!log) return;
  const g=log.groups.find(g=>g.key===key); if(!g) return;
  const ids=(g.itemIds||[]).filter(id=>QBY[id]);
  runnerCtx={ids,idx:0,key:'day_'+d+'_'+key,answered:{},origin:"pastday",pastDay:d};
  switchView("runner"); renderRunner();
}
/* Backward-compatible aliases */
function runPastDayAll(d){ runDayAll(d); }
function runPastDayRemaining(d){ runDayRemaining(d); }
function runPastGroup(d,key){ runDayGroup(d,key); }
...
function renderRunner(){
  ...
  // restore answered state: this session's pick, then this day's done,
  // then GLOBAL progress (answered on any other day counts here too)
  const prev=runnerCtx.answered[q.id];
  if(prev){
    revealAnswer(q,prev,true);
  }else{
    const dayKey = runnerCtx.pastDay || todayStr();
    const log=STATE.dayLog[dayKey];
    const doneHere = log && log.done && log.done[q.id];
    const seen = STATE.seen[q.id];
    if(doneHere || seen){
      // sync this day's progress: a question answered on ANY day counts here too
      if(seen && !doneHere && (runnerCtx.origin==="plan"||runnerCtx.origin==="pastday")){
        markDayDone(q.id);
      }
      const correctLab=(effectiveAnswer(q).match(/^([A-G])/)||[])[1];
      revealAnswer(q, correctLab||"", true);
      const note=$("#ansLine");
      if(note&&correctLab){
        const res = seen && seen.lastResult==="wrong" ? "⚠ You answered this wrong before"
                  : seen && seen.lastResult==="correct" ? "✓ You answered this correctly before"
                  : "✓ Already completed";
        note.innerHTML=`${res} · Correct answer: <b>${esc(effectiveAnswer(q))}</b>`;
      }
    }
  }
}
...
function finishRunner(){
  ...
  if(runnerCtx.origin==="review"){ switchView("review"); }
  else if(runnerCtx.origin==="pastday"){ renderDayView(runnerCtx.pastDay); }
  else if(runnerCtx.origin==="plan"){ switchView("plan"); }
